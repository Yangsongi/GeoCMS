...
function loadXML(full_url) {
	var buf_arr = full_url.split("/GeoCMS/");     // ["http://localhost:8082", "upload/20141201_140526.jpg"]
	var base_url = buf_arr[0];   // "http://localhost:8082"
	var file_name = buf_arr[1];  // "upload/20141201_140526.jpg"
	var replace_url = base_url+'/GeoCMS/';  // "http://localhost:8082/GeoCMS/"
	buf_arr = file_name.split(".");   // ["upload/20141201_140526", "jpg"]
	var xml_file_name = buf_arr[0] + '.xml';  // "upload/20141201_140526.xml"
...
function imageWrite() {
	jConfirm('뷰어를 닫고 저작을 수행하시겠습니까?', '정보', function(type){
		if(type) {
			...
			//image_content_list.jsp 의 closeImageViewer 호출
			//뷰어 닫기 수행
			window.parent.closeImageViewer();
		}
	});
}
//새창 띄우기 (저작)
function openImageWrite() {
	...
	window.open('', 'image_write_page', 'width=1170, height=860');
	var form = document.createElement('form');
	form.setAttribute('method','post');
	form.setAttribute('action','../../image_write_page.jsp');
...
<!-- 추가 객체 영역 -->
<div id="ioa_title" style='position:absolute; left:797px; top:12px; width:330px; height:245px;'><img src="images/title_02.jpg" alt="객체추가리스트"></div>
<div id='image_object_area' style='position:absolute; left:800px; top:33px; width:300px; height:245px; display:block; border:1px solid #999999; overflow-y:scroll;'>
	<table id='object_table'>
		<tr style='font-size:12px; background-color:rgb(15, 118, 207); height:25px; '>
			...
			<td width=170 height=20px align='center' style='color:#FFF;'>Data</td>
		</tr>
	</table>
</div>

<!-- EXIF 영역 -->
<div id="ex_tit"><img src="images/title_03.gif" style='position:absolute; left:799px; top:288px;' alt="이미지정보"></div>
...
<!-- EXIF 삽입 다이얼로그 객체 -->
<div id='exif_dialog' style='position:absolute; left:800px; top:310px; width:300px; height:245px; border:1px solid #999999; display:block;'>
	<div class='accordionButton'>EXIF Normal Info</div>
	<div class='accordionContent' style='height:205px; overflow-y:scroll;'>
		<table id='normal_exif_table'>
			...
			<tr><td width='15'><td><label style='font-size:12px;'>User Comment</label></td><td><input id='comment_text' name='text' type='text' style='font-size:12px;' readonly/></td></tr>
		</table>
	</div>
	
	<div class='accordionButton'>EXIF GPS Info</div>
	<div class='accordionContent' style='height:205px; overflow-y:scroll;'>
...
<!-- 저작 버튼 -->
	...
	<img src="images/write_btn.jpg" onclick='imageWrite();' style='position:absolute; left:880px; top:565px; width:140px; height:35px; display:block; cursor: pointer;'>
