...
$(function() {
	//저작 버튼 설정
	//$('.caption_button').button({ icons: { primary: 'ui-icon-grip-dotted-horizontal'} }); $('.caption_button').width(200); $('.caption_button').height(35); $('.caption_button').css('fontSize', 15); $('.caption_button').css('margin-top', 5); $('.caption_button').css('margin-left', 5); $('.caption_button').css('margin-bottom', 5);	
	//$('.speech_bubble_button').button({ icons: { primary: 'ui-icon-comment'} }); $('.speech_bubble_button').width(200); $('.speech_bubble_button').height(35); $('.speech_bubble_button').css('fontSize', 15);
	//$('.icon_button').button({ icons: { primary: 'ui-icon-heart'} }); $('.icon_button').width(200); $('.icon_button').height(35); $('.icon_button').css('fontSize', 15);	
	//$('.geometry_button').button({ icons: { primary: 'ui-icon-geometry'} }); $('.geometry_button').width(200); $('.geometry_button').height(35); $('.geometry_button').css('fontSize', 15);
...
function getAnnoList() {
	...
	$.ajax({
		type: 'POST',
		url: replace_url+'GetAnnotationListServlet',
		data: 'file_name='+encode_file_name,
		success: function(data) {
			var response = data.trim();
			setAnnoList(response);
		}
	});
}

function setAnnoList(pure_text) {
	var width = $('#image_write_canvas').attr('width');
	//var height = $('#image_write_canvas').attr('height');
	var ratio_x = img_origin_width / width;
	//var ratio_y = img_origin_height / height;
	
	var line_buf = pure_text.split("\]");
	for(var i=0; i<line_buf.length-1; i++) {
		var id = line_buf[i].split(":")[0];
		var name = line_buf[i].split(":")[1].split("\[")[0];
		var x1x2 = line_buf[i].split("\[")[1];
		var x1 = x1x2.split(",")[0];
		var x2 = x1x2.split(",")[1];
		var conv_x1 = ((img_origin_width / 2) + parseFloat(x1)) / ratio_x;
		var conv_x2 = ((img_origin_width / 2) + parseFloat(x2)) / ratio_x;
		var int_x1 = parseInt(conv_x1);
		var int_x2 = parseInt(conv_x2);
		insertAnnoTableObject(id, name, int_x1, int_x2);
	}
}

//후보군 테이블
var annoCount = 0;
function insertAnnoTableObject(id, name, x1, x2) {
	var html_text = "";
	html_text += "<tr bgcolor='#cccffc' style='font-size:12px;'>";
	html_text += "<td><input type='checkbox' id=annoCheck"+annoCount+" /></td>";
	html_text += "<td align='center'><label id='annoId"+annoCount+"'>"+id+"</label></td>";
	html_text += "<td align='center'><label id='annoName"+annoCount+"'>"+name+"</label></td>";
	html_text += "<td><label id='annoX1"+annoCount+"'>"+x1+"</label></td>";
	html_text += "<td><label id='annoX2"+annoCount+"'>"+x2+"</label></td>";
	html_text += "</tr>";

	$('#candidate_table tr:last').after(html_text);
...
function addAnno() {
	for(var i=0; i<annoCount; i++) {
		//var anno_check = $('input[name=annoCheck'+i+']').attr('checked');
		var anno_check = $('#annoCheck'+i).attr('checked');
		if(anno_check) {
			var text = $('#annoName'+i).html();
			var height = $('#image_write_canvas').attr('height');
			var img_left_str = $('#image_write_canvas').css('left');
			img_left_str = img_left_str.replace('px','');
			var img_left = parseInt(img_left_str);
			var left = ((parseInt($('#annoX1'+i).html()) + parseInt($('#annoX2'+i).html())) / 2) + img_left;
			autoCreateText('b', 'Normal', '#ffffff', 'none', 'false', 'false', 'false', 'false', text, height/2, left);
		}
	}
}
...
function saveImageWrite(type) {
	...
	if(type==1 || type==2) {
		...
		//alert(xml_text);
		//var xmlObject = (new DOMParser()).parseFromString(xml_text, "text/xml");
...
function resizeMap() {
	if(resize_map_state==1) {
		...
		resize_map_state=2;
		$('#image_map_area').animate({left:init_map_left-resize_scale, top:init_map_top-resize_scale, width:init_map_width+resize_scale, height:init_map_height+resize_scale},"slow", function() { $('#resize_map_btn').css('background-image','url(../../images/icon_map_min.jpg)'); reloadMap(1); });
	}
	else if(resize_map_state==2) {
		resize_map_state=1;
		$('#image_map_area').animate({left:init_map_left, top:init_map_top, width:init_map_width, height:init_map_height},"slow", function() { $('#resize_map_btn').css('background-image','url(../../images/icon_map_max.jpg)'); reloadMap(1); });
	}
	else {}
}
//객체 테이블
function insertTableObject(data_arr) {
	var html_text = "";
	html_text += "<tr id='obj_tr"+data_arr[0]+"' bgcolor='#EAEAEA' style='font-size:12px; height:18px; color:rgb(21, 74, 119);'>";
...
<!------------------------------------------------------ 화면 영역 ----------------------------------------------------------->
<!-- 저작 버튼 영역 -->
<div class='image_write_function' style='position:absolute; left:10px; top:2px; width:1125px; height:65px; display:block; background:url("images/mini_Bar.jpg") repeat-x 0 0;'>
		...
		<img src="images/caption_btn.jpg" onclick='inputCaption(0,"");' style="cursor: pointer; margin-left: 10px;">
		<img src="images/speech_btn.jpg" onclick='inputBubble(0,"");' style="cursor: pointer; margin-left: 10px;">
		<img src="images/image_btn.jpg" onclick='inputIcon();' style="cursor: pointer; margin-left: 10px;">
		<img src="images/geo_btn.jpg" onclick='inputGeometry();' style="cursor: pointer; margin-left: 10px;">
		...
		<img src="images/clo_btn.jpg" onclick='closeImageWrite();' style="float:right; margin:12px; cursor: pointer;">
		<img src="images/sav_btn.jpg" onclick='saveSetting();' style="float:right; margin:12px; cursor: pointer;">
...
<!-- 후보군 영역 -->
<!-- <div id='image_candidate_area' style='position:absolute; left:820px; top:65px; width:310px; height:245px; display:block; border:1px solid #999999; overflow-y:scroll;'>
	<table id='candidate_table'>
		<tr bgcolor='#16193c' style='font-size:12px;'>
			<td width=10 align='center'></td>
			<td width=40 align='center' style='color:#FFF;'>ID</td>
			<td width=90 align='center' style='color:#FFF;'>Name</td>
			<td width=50 align='center' style='color:#FFF;'>X1</td>
			<td width=50 align='center' style='color:#FFF;'>X2</td>
		</tr>
	</table>
</div> -->
<!-- <button class="ui-state-default ui-corner-all" style="position:absolute; left:1065px; top:290px; width:50px; height:20px; font-size:11px;" onclick="addAnno();">추가</button> -->

<!-- 객체추가리스트 -->
<div id="ioa_title" style='position:absolute; left:800px; top:73px; width:330px; height:245px;'><img src="images/title_02.jpg" alt="객체추가리스트"></div>
<div id='image_object_area' style='position:absolute; left:800px; top:92px; width:330px; height:245px; display:block; border:1px solid #999999; overflow-y:scroll;'>
	<table id='object_table'>
		<tr  style='font-size:12px; background-color:rgb(15, 118, 207); height:25px; '>
			<td width=50 height=20px align='center' style='color:#FFF;'>ID</td>
			<td width=80 height=20px align='center' style='color:#FFF;'>Type</td>
			<td width=180 height=20px align='center' style='color:#FFF;'>Data</td>
		</tr>
	</table>
</div>
...
<!-- EXIF 영역 -->
<div id="ex_title"><img src="images/title_03.gif" style='position:absolute; left:800px; top:345px;' alt="이미지정보"></div>
<div id='image_exif_area' style='position:absolute; left:800px; top:365px; width:330px; height:185px; display:block; border:1px solid #999999;'>
</div>

<!-- 지도 영역 -->
<div id="ima_title"><img src="images/title_04.gif" style='position:absolute; left:802px; top:557px;' alt="지도"></div>
<div id='image_map_area' style='position:absolute; left:800px; top:580px; width:330px; height:270px; display:block;'>
	<iframe id='googlemap' src='../maps/image_googlemap.jsp' style='width:100%; height:100%; margin:1px; border:none;'></iframe>
	...
	<div style='display:table; width:100%; height:100%;'>
		<div align="center" style='display:table-cell; vertical-align:middle;'>
			<table border='0'>
				...
				<!-- <td><input id="caption_bg_color" type="text" class="iColorPicker" value="#000000" style="width:50px;"/></td>  원래이거-->
...
<!-- EXIF 삽입 다이얼로그 객체 -->
<div id='exif_dialog' style='position:absolute; left:800px; top:365px; width:330px; height:185px; border:1px solid #999999; display:block;'>
	<div class='accordionButton'>&nbsp;EXIF</div>
	<div class='accordionContent' style='height:150px; overflow-y:scroll;'>
	...
	<div class='accordionButton'>&nbsp;EXIF-GPS</div>
	<div class='accordionContent' style='height:150px; overflow-y:scroll;'>
		<table id='gps_exif_table'>
